...
def extract_node_info(node, source_title, node_to_layer):
    ...
    try:
        ...
        if isinstance(vector, (list, tuple, np.ndarray)):
            if len(vector) > 0:
                vector_str = json.dumps(vector.tolist() if hasattr(vector, 'tolist') else list(vector))
            else:
                vector_str = "[]"
        else:
            vector_str = "[]"
        ...
        print(f"Error extracting node info from node {getattr(node, 'index', 'unknown')}: {e}")
        return None

def main():
    """메인 함수"""
    recursive_dir = Path("results/recursive")
    output_file = "results/recursive_combined_raptor_data.csv"
    
    if not recursive_dir.exists():
        print(f"Directory {recursive_dir} does not exist!")
        return
    
    # 모든 pkl 파일 찾기
    pkl_files = list(recursive_dir.glob("*.pkl"))
    ...
    # 컬럼 순서 맞추기
    columns = ['source_title', 'node_index', 'ko_content', 'ko_content_vector', 'children', 'layer', 'is_root', 'is_leaf']
    df = df[columns]
